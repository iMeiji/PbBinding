        ...
        return buildCodeBlock {
            ...
            type.oneOfs()
                ...
                .forEach { oneOf ->
                    ...
                    val fieldNames =
                        oneOf.fields().joinToString(", ", transform = nameAllocator::get)
    ...
    private fun generateBuilderClass(
        ...
        val returnBody = buildCodeBlock {
            ...
            val missingRequiredFields =
                MemberName("com.squareup.wire.internal", "missingRequiredFields")
            type.fieldsAndOneOfFields().forEach { field ->
                val fieldName = nameAllocator[field]
    ...
    private fun builderSetter(
        ...
        if (field.isRepeated) {
            val checkElementsNotNull =
                MemberName("com.squareup.wire.internal", "checkElementsNotNull")
...
//            .build())
        ...
        val returnBody = buildCodeBlock {
            ...
            message.fieldsAndOneOfFields().forEach { field ->
                ...
                val throwExceptionBlock =
                    if (!field.isRepeated && !field.isMap && field.isRequired) {
                        CodeBlock.of(" ?: throw %1M(%2L, %2S)", missingRequiredFields, field.name())
                    } else {
                        CodeBlock.of("")
                    }
    ...
    private fun generateEnum(message: EnumType): TypeSpec {
        val className = message.type().typeName
        val nameAllocator = nameAllocator(message)
